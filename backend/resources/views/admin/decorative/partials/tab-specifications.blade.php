                <div id="pane-specifications" class="wizard-pane">
                    ...
                    <div class="alert" style="background:#eef2ff; border:1px solid #c7d2fe; color:#3730a3; border-radius:8px; font-size:13px; padding:10px 14px; margin-bottom:16px;">
                        <i class="ft-info"></i> Specifications are <strong>variant-wise</strong>. Select a variant to add or edit its specifications.
                    </div>

                    {{-- Variant Card Grid --}}
                    <div class="mb-3">
                        <label class="font-weight-bold mb-2 d-block" style="font-size:13px;">Select Variant</label>
                        <div class="d-flex flex-wrap" id="spec-variant-cards" style="gap:10px;">
                            @forelse($product->variants as $variant)
                                <div class="spec-variant-card" data-id="{{ $variant->id }}" data-status="{{ $variant->status }}"
                                    style="display:flex; align-items:center; gap:10px; padding:10px 14px; border:2px solid #e5e7eb; border-radius:8px; cursor:pointer; background:#fff; min-width:160px; transition:all .15s;">
                                    <div style="width:38px; height:38px; border-radius:6px; background:{{ $variant->colorMaster ? $variant->colorMaster->hex_code ?? '#ddd' : '#ddd' }} url('{{ $variant->main_image ? asset('storage/uploads/decorative/'.$variant->main_image) : '' }}') center/cover; flex-shrink:0; border:1px solid #eee;"></div>
                                    <div>
                                        <div style="font-size:13px; font-weight:600; line-height:1.2;">{{ $variant->name }}</div>
                                        @if($variant->sku)<div style="font-size:11px; color:#9ca3af;">{{ $variant->sku }}</div>@endif
                                    </div>
                                </div>
                            @empty
                                <div class="alert alert-secondary p-2" style="font-size:13px;">No variants found. Add variants first.</div>
                            @endforelse
                        </div>
                    </div>

                    {{-- Specs content area --}}
                    <div id="spec-content-area" style="display:none;">

                        {{-- Header --}}
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <div>
                                <h6 class="m-0" style="font-size:15px; font-weight:700;">Specifications for <span id="spec-variant-name"></span>
                                    <span id="spec-variant-status-badge" class="badge ml-1" style="font-size:10px; vertical-align:middle;"></span>
                                </h6>
                                <small class="text-muted">Manage all specification details for this variant.</small>
                            </div>
                        ...
                        <div class="card shadow-sm border-0 mb-3">
                            <div class="card-header bg-white d-flex justify-content-between align-items-center" style="padding:12px 16px;">
                                ...
                                <button type="button" class="btn btn-sm btn-outline-primary btn-add-spec" data-section="basic_specifications" style="font-size:12px; padding:4px 10px;">
                            ...
                            <div class="card-body p-0">
                                <table class="table mb-0 spec-table" style="font-size:13px;">
                                    ...
                                    <tbody class="spec-tbody" data-section="basic_specifications" id="spec-tbody-basic">
                                    </tbody>
                                </table>
                        ...
                        <div class="card shadow-sm border-0">
                            ...
                            <div class="card-body p-0">
                                <table class="table mb-0 spec-table" style="font-size:13px;">
                                    ...
                                    <tbody class="spec-tbody" data-section="dimensions" id="spec-tbody-dimensions">
                                    </tbody>
                                </table>
                                <div class="p-4 text-center text-muted no-specs-msg" id="no-specs-dimensions" style="display:none; font-size:13px;">
                                    <i class="ft-maximize font-large-1 d-block mb-1"></i>No rows yet. Click "Add Row" to start.
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="spec-placeholder" class="card shadow-sm border-0 text-center py-5">
                        <i class="ft-list font-large-2 text-muted mb-2 d-block"></i>
                        <h5 class="text-muted">Select a variant above to manage its specifications</h5>
                    </div>
