                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div id="gradient-fields" style="display: none;">
                                ...
                                <input type="hidden" id="gradient_start" name="gradient_start" value="{{ old('gradient_start', $color->gradient_start ?? '#A84628') }}">
                                <input type="hidden" id="gradient_end" name="gradient_end" value="{{ old('gradient_end', $color->gradient_end ?? '#000000') }}">
                                <input type="hidden" id="gradient_type" name="gradient_type" value="{{ old('gradient_type', $color->gradient_type ?? 'linear') }}">
                                <input type="hidden" id="gradient_direction" name="gradient_direction" value="{{ old('gradient_direction', $color->gradient_direction ?? '135deg') }}">
                                        ...
                                        <div class="p-2 mb-3 bg-light rounded d-flex flex-wrap align-items-center justify-content-between border" id="activeStopBar">
                                            <div class="d-flex align-items-center flex-wrap mr-2">
                                                ...
                                                <div class="d-flex align-items-center mr-3 my-1">
                                                    ...
                                                    <input type="text" id="activeStopColorHex" class="form-control form-control-sm ml-1" style="width: 85px; font-family: monospace; font-weight: 600; text-transform: uppercase;" maxlength="7" value="{{ $color->gradient_start ?? '#A84628' }}">
                                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="2">{{ old('description', $color->description) }}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="order">Display Order</label>
                                <input type="number" class="form-control" id="order" name="order" 
                                       value="{{ old('order', $color->order) }}" min="0">
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="form-group mb-0">
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" id="is_active" 
                                           name="is_active" value="1" {{ old('is_active', $color->is_active) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initial Gradient State from model or defaults
    let gradientType = '{{ old("gradient_type", $color->gradient_type ?? "linear") }}';
    let gradientDirection = '{{ old("gradient_direction", $color->gradient_direction ?? "135deg") }}';
    ...
    // Default Stops from model
    let initialStart = '{{ old("gradient_start", $color->gradient_start ?? "#A84628") }}';
    let initialEnd = '{{ old("gradient_end", $color->gradient_end ?? "#000000") }}';
    if (!initialStart || initialStart === '') initialStart = '#A84628';
    if (!initialEnd || initialEnd === '') initialEnd = '#000000';

    let stops = [
        { color: initialStart, position: 0 },
        { color: initialEnd, position: 100 }
    ];
