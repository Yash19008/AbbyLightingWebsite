@extends('admin.page')
@section('title', 'Add Parameter Card — ' . $collection->name)
...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6"><h1>Add Parameter Card</h1></div>
            <div class="col-sm-6">
...
<section class="content">
    <div class="container-fluid">
        <form action="{{ route('admin.collections.parameter-items.store', $collection->slug) }}" method="POST">
            @csrf
            <div class="row">
                <div class="col-md-8">
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror"
                                          ...
                                          placeholder="Full description paragraph…">{{ old('description') }}</textarea>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="d-flex align-items-center" style="gap:8px">
                                            <span id="bg_color_swatch" style="display:inline-block;width:32px;height:32px;border-radius:4px;border:1px solid #ccc;background:{{ old('bg_color', '#a84628') }};flex-shrink:0;"></span>
                                            <select class="form-control" id="bg_color" name="bg_color"
                                                ...
                                                @foreach($colorMasters as $color)
                                                    <option value="{{ $color->hex_code }}" {{ old('bg_color', '#a84628')==$color->hex_code?'selected':'' }}>
                                                        {{ ucfirst($color->name) }} ({{ $color->hex_code }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <small class="form-text text-muted">Default background color</small>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="d-flex align-items-center" style="gap:8px">
                                            <span id="hover_swatch" style="display:inline-block;width:32px;height:32px;border-radius:4px;border:1px solid #ccc;background:{{ old('hover_bg_color', '#000000') }};flex-shrink:0;"></span>
                                            <select class="form-control" id="hover_bg_color" name="hover_bg_color"
                                                ...
                                                <option value="">— No Color —</option>
                                                @foreach($colorMasters as $color)
                                                    <option value="{{ $color->hex_code }}" {{ old('hover_bg_color', '#000000')==$color->hex_code?'selected':'' }}>
                                                        {{ ucfirst($color->name) }} ({{ $color->hex_code }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <small class="form-text text-muted">Color when card is hovered</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @error('order') is-invalid @enderror"
                                       id="order" name="order"
                                       value="{{ old('order', $collection->parametersSection ? $collection->parametersSection->items->count() + 1 : 1) }}"
                        ...
                        <div class="card-footer">
                            <button type="submit" class="btn btn-success">
                                <i class="fas fa-save mr-1"></i> Save Parameter Card
                            </button>
